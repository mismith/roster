...
angular.module('roster-io', ['ngOpbeat', 'ui.router', 'ui.router.title', 'ngMaterial', 'firebaseHelper', 'ngTouch']).config(["$opbeatProvider", "$locationProvider", "$urlRouterProvider", "$urlMatcherFactoryProvider", "$stateProvider", "$firebaseHelperProvider", "$sceProvider", "$compileProvider", function ($opbeatProvider, $locationProvider, $urlRouterProvider, $urlMatcherFactoryProvider, $stateProvider, $firebaseHelperProvider, $sceProvider, $compileProvider) {
	...
	.state('home', {
		...
		resolve: {
			authed: ["Auth", "$state", function authed(Auth, $state) {
		...
		resolve: {
			Roster: ["$firebaseHelper", "$stateParams", "$state", function Roster($firebaseHelper, $stateParams, $state) {
			...
			$title: ["Roster", function $title(Roster) {
				return Roster.name;
			}]
		}
	}).state('event', {
		...
		resolve: {
			...
			$title: ["Roster", "Event", function $title(Roster, Event) {
		...
		resolve: {
			Invite: ["$firebaseHelper", "$stateParams", "$state", function Invite($firebaseHelper, $stateParams, $state) {
			...
			Inviter: ["$firebaseHelper", "$stateParams", "Invite", function Inviter($firebaseHelper, $stateParams, Invite) {
				return $firebaseHelper.object('data/users', Invite.inviterId).$loaded();
			}],
			Roster: ["$firebaseHelper", "$stateParams", "Invite", function Roster($firebaseHelper, $stateParams, Invite) {
		...
		controller: 'UserCtrl',
		resolve: {
			User: ["$firebaseHelper", "$stateParams", "$state", function User($firebaseHelper, $stateParams, $state) {
			...
			$title: ["User", function $title(User) {
				return User.name;
			}]
		}
	})
	...
	$rootScope.canEdit = function () {
		...
		return !!args.filter(function (test) {
			if (angular.isArray(test) && test.indexOf($rootScope.$me.$id) >= 0) return true;else if (angular.isObject(test) && test[$rootScope.$me.$id]) return true;else return test ? test === $rootScope.$me.$id : false;
	...
	$scope.deleteRoster = function (skipConfirm) {
		if (skipConfirm || confirm('Are you sure you want to permanently delete this roster?')) {
			...
			var deferreds = [],
			    rosterId = Roster.$id,
			    removeLink = function removeLink(uid) {
	...
	// invites
	var sendInviteEmail = function sendInviteEmail(inviteId) {
		var deferred = $q.defer(),
...
}]).controller('UserCtrl', ["$scope", "$firebaseHelper", "$state", "$mdToast", "$mdDialogForm", "User", "$q", function ($scope, $firebaseHelper, $state, $mdToast, $mdDialogForm, User, $q) {
	$scope.user = User;
 ...
 */
	...
	return function ($event, options) {
		...
		var $statuses = $firebaseHelper.object('constants/statuses'),
		    // constant
		...
		return {
			statuses: $statuses,
			rsvps: $rsvps,
			getParticipantStatus: function getParticipantStatus(participantId) {
				return $rsvps && $rsvps[participantId] ? $rsvps[participantId].status : -2;
			},
			setParticipantStatus: function setParticipantStatus(participantId, status) {
	...
	return {
		...
		controller: ["$scope", "RSVP", function controller($scope, RSVP) {
			$scope.RSVP = RSVP;
		}]
	};
}).directive('avatar', ["$parse", function ($parse) {
	return {
		scope: {
			roster: '='
		},
	...
	return {
		show: function show(options) {
			return $mdDialog.show($mdDialog.confirm(angular.extend({
				...
				ok: 'Submit',
				cancel: 'Cancel',
				template: ['<md-dialog ng-form="dialogForm" md-theme="{{ dialog.theme }}" aria-label="{{ dialog.ariaLabel }}" ng-class="{loading: dialog.loading}">', '<md-dialog-content role="document" tabIndex="-1">', '<h2 class="md-title">{{ dialog.title }}</h2>', '<p ng-if="dialog.content">{{ dialog.content }}</p>', '<div ng-if="dialog.contentUrl" ng-include="dialog.contentUrl"></div>', '</md-dialog-content>', '<div class="md-actions">', '<md-button ng-if="dialog.$type == \'confirm\'" ng-click="dialog.abort()" class="md-primary">', '{{ dialog.cancel }}', '</md-button>', '<md-button type="submit" ng-disabled="dialogForm.$invalid || dialog.loading" ng-click="dialog.startLoading(); dialog.onSubmit(this).then(dialog.hide).finally(dialog.stopLoading)" class="md-primary">', '{{ dialog.ok }}', '</md-button>', '</div>', '<md-dialog-footer>', '<md-progress-circular md-mode="indeterminate"></md-progress-circular>', '</md-dialog-footer>', '</md-dialog>'].join(''),
	...
	var BASE_URL = '/api/v1/';
	return {
		post: function post(method, config) {
			var deferred = $q.defer();

			$http.post(BASE_URL + method.replace(/^\/+/, ''), config.data, config).success(function (res) {
				if (res && res.success) {
	...
	return {
		...
		link: function link($scope, $element, $attrs) {
			...
			var $inner = angular.element($element.children()[0]),
			    // @HACKY
